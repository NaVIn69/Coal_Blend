...
import { Upload, FileText, File, CheckCircle, AlertCircle, Save, X } from "lucide-react";
...
const COAL_PROPERTIES = [
  ...
  { id: "VM", label: "Volatile Matter %", type: "number", step: "0.01", required: true },
  { id: "FC", label: "Fixed Carbon %", type: "number", step: "0.01", required: true },
  { id: "S", label: "Sulfur %", type: "number", step: "0.01", required: true },
  { id: "P", label: "Phosphorus %", type: "number", step: "0.01", required: true },
  { id: "SiO2", label: "SiO2 %", type: "number", step: "0.01", required: true },
  { id: "Al2O3", label: "Al2O3 %", type: "number", step: "0.01", required: true },
  { id: "Fe2O3", label: "Fe2O3 %", type: "number", step: "0.01", required: true },
  ...
  { id: "Mn3O4", label: "Mn3O4 %", type: "number", step: "0.01", required: true },
  ...
  { id: "N", label: "Nitrogen %", type: "number", step: "0.01", required: true },
...
function VendorDataUpload() {
  ...
  const handleFileChange = async (e) => {
    ...
    if (selectedFile.type === "application/pdf") {
      ...
      try {
        // Parse the PDF
        const parsedData = await parseCoalPdf(selectedFile);
        
        // Filter out null/undefined values and update form data
        ...
        setFormData(prev => ({
          ...prev,
          ...validData,
          coal_name: validData.coal_name || prev.coal_name || selectedFile.name.replace(/\.pdf$/i, '').trim(),
        }));
  
        setMessage({
          type: "success",
          text: "PDF parsed successfully. Please review the extracted data before submitting.",
        });
      } catch (error) {
        console.error('Error parsing PDF:', error);
        setMessage({
          type: "error",
          text: "Failed to parse PDF. Please try manual entry or upload a different file.",
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        setMessage({
          type: "success",
          text: response.data.message || "Vendor data uploaded successfully and is pending approval",
        });
        setFormData({});
        setFile(null);
      } else {
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          ...
          {isManualEntry ? (
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
              ...
              <p className="text-gray-600 mb-2">Upload a PDF containing coal property data</p>
              <p className="text-gray-500 text-sm mb-4">
                The PDF should contain a table with coal properties in a standardized format
              </p>
              ...
              <div className="flex flex-col items-center">
                ...
                >
                  {isLoading ? (
                    ...
                    <>
                      <Upload className="h-4 w-4 mr-1" /> Upload and Extract Data
                    </>
                  )}
                </button>
              </div>
              ...
              <div className="mt-8 text-left bg-gray-50 p-4 rounded-md">
                ...
                <ul className="list-disc pl-5 text-sm text-gray-600 space-y-1">
                  ...
                  <li>Include all required properties: Ash, VM, FC, S, P, etc.</li>
                </ul>
              </div>
            </div>
          )}
